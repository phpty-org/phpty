...
`run_for_operators` / `process_key` / `wrap_method_call` were stubbed with
follow-the-diff no-ops (ADR-0015). Tier 5 fills them in and lands the two vi
keymaps.

- **The two generated vi tables.** `src/KeyActor/ViInsert.php` and
  `src/KeyActor/ViCommand.php` are generated by `bin/generate_vi_mapping.php` — the
  sibling of `generate_emacs_mapping.php`, same parse and emitter over the two
  256-slot upstream tables, per-slot `# NNN key` comments preserved for
  line-for-line diffability, idempotent output. Do not hand-edit; regenerate.
  `Config` now wires all three default keymaps (emacs / vi_insert / vi_command) plus
  their empty inputrc-additions layers, so the ANSI gate registers for every keymap
  unconditionally as upstream does. `Config::editing_mode_label()` exposes the
  active label; `Config::reset()` flips vi_command back to vi_insert on accept
  (config.rb:35). The KeyStroke vi-ESC handling (`match_unknown_escape_sequence`'s
  `ESC char` rejection) was already present from tier 0 and now engages once the
  mode is vi.
- **Operator composition, ported from line_editor.rb:921-1008,2062-2123.** A pending
  operator (`d`/`c`/`y`) is held in `@vi_waiting_operator` as the confirm-method
  name (`vi_delete_meta_confirm` etc.); the following motion runs first, then
  `run_for_operators` restores `@byte_pointer` and applies the operator over the
  byte range the motion covered. Counts multiply (`2d3w` → 6): `@vi_arg`
  (ed_argument_digit / vi_zero) times `@vi_waiting_operator_arg`. `f`/`F`/`t`/`T`
  install a `@waiting_proc`, so the operator defers until the follow-up key resolves
  the range (`df<char>`). Non-motion keys and csi keys cancel the operator
  (`process_key`'s VI_WAITING_ACCEPT_METHODS / VI_MOTIONS gate). `@drop_terminate_
  spaces` makes `cw` behave like `ce`.
- **Ruby keyword-arg reflection → PHP parameter-name reflection.** Upstream's
  `wrap_method_call` reflects on a method's `parameters` to decide whether to pass
  `arg:`/`inclusive:` (line_editor.rb:953-983). PHP has no keyword arguments, so the
  motions/editors take positional `int $arg = 1` / `bool $inclusive = false` params
  and `method_arg_shape()` reflects on those **parameter names** (cached per method)
  to build the call the same way — the one idiom mapping new to this tier.
- **Vi clipboard vs kill ring.** `p`/`P` paste `@vi_clipboard` (set by `copy_for_vi`
  on `x`/`X`/`d`/`c`/`y`), a separate register from the emacs `@kill_ring` that
  `C-y` yanks — upstream keeps them distinct and so does the port.
- **`vi_histedit` (`v`) with an injectable editor.** The temp-file dump / `$EDITOR`
  spawn / re-read structure is ported, but the editor invocation is injectable
  (`set_vi_histedit_invocation`) so tests exercise the structure without spawning a
  real editor; its ScreenTest is skipped, as the tier brief directs. `vi_alias`
  (`@`) and `vi_comment_out` (`#`) are referenced by the command table but
  unimplemented upstream, so they dispatch to nothing via `wrap_method_call`'s
  method-exists guard (the ed_unassigned-equivalent) and are covered by the
  `unimplemented_vi_command_is_no_op` case.
- **The deferred-from-tier-3 prefix history search.** `ed_search_prev_history` /
  `ed_search_next_history` (history-search-backward/forward) and their
  `search_history` helper are ported now, closing the tier-3 deferral. **Neither the
  emacs nor the vi keymap of gem 0.6.3 binds a key to them** (verified: no
  `ed_search_*` slot in emacs.rb, vi_insert.rb, or vi_command.rb — upstream's own
...
- `src/KeyActor/Emacs.php` ← `bin/generate_emacs_mapping.php`, which parses the
  ...
  diffability.
- `src/KeyActor/ViInsert.php` and `src/KeyActor/ViCommand.php` ←
  `bin/generate_vi_mapping.php`, the sibling generator that does the same over
  `VI_INSERT_MAPPING` and `VI_COMMAND_MAPPING` (one generator, both files).
...
- `ViMappingTest` — integrity checks on the two generated vi tables and the Base
  trie that indexes them (the vi sibling of `EmacsMappingTest`): the printable →
  ed_insert bulk of vi_insert, the vi_command motions/operators, and the
  referenced-but-unimplemented `vi_alias` / `vi_comment_out` slots.
- `LineEditorViTest` — the method cases of `test_key_actor_vi.rb`: the mode
  switches (i/a/I/A/ESC and the emacs escape hatch), motions
  (h/l/w/b/e/W/B/E/f/F/t/T/0/^/| with counts, incl. wide-char and combining-mark
  runs), editing (x/X/r/p/P/C/^U/^W/^H, quoted-insert with a vi arg), and the full
  d/c/y operator-composition suite (counts, arg-including-zero, waiting-proc
  motions, dc/dy/cd/cy cancellation, csi-key cancellation, the no-op unimplemented
  command). Vi mode is set programmatically in setup (the inputrc path is tier 7).
- `ReadlineScreenTest` (tier-5 cases) — two ScreenTest scenarios on a real Pty via
